...
function initEmailClient() {
  if (window.emailjs && typeof window.emailjs.init === "function") {
    window.emailjs.init("W562KgMPucGcnO9vy");
...
function initContactFormValidation() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    const payload = { name, company, email, phone, service, description };
    ...
    if (emailClientReady && window.emailjs?.send) {
      setSubmitting(true);
      window.emailjs
        .send("service_cexy257", "template_ex2v73g", payload)
        .then(() => {
          form.reset();
          clearErrors();
          showSuccess();
      ...
      alert("No se pudo enviar el correo porque el servicio de envío no está disponible.");
    }
  });
